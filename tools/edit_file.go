...
func EditFile(args map[string]any) ToolResult {
	...
	if oldStr == "" {
		return ToolResult{Output: "错误: old_string 不能为空", Success: false, Error: "old_string 不能为空", FailureCategory: FailureCategoryInvalidArgument}
	}
	if oldStr == newStr {
		return ToolResult{Output: "错误: new_string 必须与 old_string 不同", Success: false, Error: "new_string 必须与 old_string 不同", FailureCategory: FailureCategoryInvalidArgument}
	}
	...
	if err != nil {
		return ToolResult{Output: fmt.Sprintf("读取失败: %v", err), Success: false, Error: "文件读取失败", FailureCategory: FailureCategoryNotFound, FailureHint: "文件读取失败。先确认路径与文件是否存在,再重试。"}
	}
